...
import { initializeCollaboration, joinSession, isCollaborating, getConnectedPeers } from '../utils/collaboration';
...
watch(() => magazineStore.articles || defaultArticles.value, (articles) => {
  ...
  const maxPage = articles.reduce((max, article) => {
    if (!article.startPage || !article.pageCount) return max;
...
const calculateStartPage = (newArticle: any) => {
  ...
  const lastArticle = [...articles].sort((a, b) => {
    if (!a.startPage || !a.pageCount || !b.startPage || !b.pageCount) return 0;
    const aEndPage = a.startPage + a.pageCount - 1;
    const bEndPage = b.startPage + b.pageCount - 1;
    return bEndPage - aEndPage;
  })[0];

  if (!lastArticle.startPage || !lastArticle.pageCount) return 1;
    ...
    <div v-else-if="loadError" class="flex items-center justify-center min-h-screen bg-gray-100">
      <div class="w-full max-w-md p-8 space-y-6 text-center bg-white shadow-lg rounded-xl">
        <div class="flex justify-center">
          <div class="w-16 h-16 text-red-600">⚠️</div>
        </div>
        <div class="space-y-2">
          <h2 class="text-2xl font-bold text-gray-900">Error Loading Magazine</h2>
    ...
    <div v-else class="mx-auto max-w-7xl">
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-4">
          <h2 class="text-2xl font-semibold text-gray-900">Magazine Layout</h2>
          <CollaborationStatus 
            :isCollaborating="collaborationStatus.isCollaborating"
            :peersCount="collaborationStatus.peersCount"
            :isEditingAllowed="magazineStore.allowEdit"
          />
        </div>
        <Toolbar
      ...
      <MainContent
        ...
        :isEditingAllowed="!magazineStore.isShared || magazineStore.allowEdit"
      ...
      <Modal :isOpen="isModalOpen" @close="() => { isModalOpen = false; editingArticle = null; }">
        <ArticleForm 
